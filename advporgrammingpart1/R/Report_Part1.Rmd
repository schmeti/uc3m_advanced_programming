...
library(rpart)
library(rpart.plot)
play_tennis = read.csv("C:/Users/flore/Desktop/git/advance_programming/uc3m_advanced_programming/play_tennis.csv",
                       header=TRUE)
```
### 2 and 3
...
```
As you can the both code give the same result.
### 7
Next we will be adding a variable call "Energy_Level" that have High, Low or Neutral.
...
play_tennis
```
This variable should be the new best_feature and the gini impurity should be theorically 0.3071429.
...
### 9
```{r}
source("C:/Users/flore/Desktop/git/advance_programming/uc3m_advanced_programming/advporgrammingpart1/R/r_version_of_ds_v2.R")
fit_decision_stump_R_v2(play_tennis[c("Outlook","Temperature","Humidity","Wind")],play_tennis$PlayTennis)
```

```{r}
library(Rcpp)
sourceCpp("C:/Users/flore/Desktop/git/advance_programming/uc3m_advanced_programming/advporgrammingpart1/src/decision_stump_c_v2.cpp")
```
...
```
Both function create the same branch for the tree and also have the same mayority category.